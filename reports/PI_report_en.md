...
1. **API Server (`src/index.ts`, `src/api.ts`)**: Hono-based REST API server with endpoint for query submission
2. **Query Handler (`src/run_query.ts`)**: Main query processing pipeline that orchestrates safety checks, LLM API calls, and logging
3. **Safety Module (`src/safety/`)**: Modular safety system with input validation (`prompt.ts`) and response parsing (`response.ts`)
4. **Logging Module (`src/metrics.ts`)**: Unified logging system that tracks metrics and safety checks
5. **Prompt Template (`prompts/main_prompt.md`)**: Instruction-based prompt with JSON schema definition

### Architecture Flow

```
User Question → Safety Check → Query Sanitization → LLM API Call → 
Response Parsing → Validation → Metrics Logging → JSON Output
```
...
- `query`: Truncated user query for reference
...
**Solution**: 
- Implemented cost calculation based on model pricing tables
- Log all metrics to JSON file for analysis
...
- Detects common attack patterns (e.g., "ignore previous instructions")
- Blocks high-risk queries before API call
- Comprehensive logging of all safety checks to `reports/safety-reports/` for audit trail
- Unified logging ensures safety checks are tracked alongside metrics
...
8. **Log Aggregation**: Centralize logs from metrics and safety reports for unified analysis
...
- **REST API** built on Hono framework for easy integration and deployment
- **Structured JSON responses** for reliable downstream integration
- **Unified logging system** that tracks both metrics and safety checks comprehensively
- **Modular architecture** with separated concerns (safety, metrics, API) for easy maintenance and extension
- **Safety mechanisms** with dedicated logging to prevent and track adversarial attacks

The instruction-based prompt technique with JSON schema proves effective for ensuring consistent, parseable output while maintaining response quality. The dual logging system (metrics + safety checks) provides complete observability for both operational and security concerns. The Hono-based API architecture ensures the system is ready for deployment across various environments (Node.js, Edge, Cloudflare Workers) with minimal modifications.
